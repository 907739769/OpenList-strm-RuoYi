...
import {
  ...
  Location, Clock, VideoPlay, CopyDocument, Plus, Edit, Delete, MagicStick, InfoFilled
...
const handleExecuteOne = async (row: any) => {
  try {
    await ElMessageBox.confirm(`是否确认执行重命名任务"${row.sourceFolder}"？`, '提示', { type: 'warning' })
...
const handleBatchExecute = async () => {
  try {
    await ElMessageBox.confirm(`是否确认批量执行选中的 ${selectedIds.value.length} 个重命名任务？`, '提示', { type: 'warning' })
...
const handleDelete = async (row?: any) => {
  ...
  try {
    await ElMessageBox.confirm(`是否确认删除重命名任务"${row.sourceFolder}"？`, '警告', { type: 'warning' })
    await deleteRenameTaskApi(id)
