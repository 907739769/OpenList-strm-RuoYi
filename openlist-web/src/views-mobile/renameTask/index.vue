<template>
  <div class="mobile-page">
    ...
    <div class="task-list">
      <div v-for="task in renameTasks" :key="task.taskId" class="task-card">
        ...
        <div class="task-card-body">
          <div class="task-meta"><el-icon><Folder /></el-icon> {{ task.homePath }}</div>
          <div class="task-meta"><el-icon><Document /></el-icon> 模式: {{ task.renameMode }}</div>
        </div>
        <div class="task-card-footer">
          <span class="task-time">创建: {{ task.createTime }}</span>
        </div>
      </div>
      <el-empty v-if="renameTasks.length === 0" description="暂无重命名任务" />
    </div>
  </div>
</template>
