...
public class OpenlistCopyTaskRestController extends BaseController
{
    ...
    {
        startPage();
        List<OpenlistCopyTaskPlus> list = openlistCopyTaskPlusService.list(buildQueryWrapper(openlistCopyTask));
        long total = PageHelper.count(() -> openlistCopyTaskPlusService.list(buildQueryWrapper(openlistCopyTask)));
        int page = getPage();
